...
- React 19、TypeScript 与 Vite 构成 Web 客户端；身份、项目、任务与 `/projects/:projectId/knowledge` 的资源、搜索、按需引用上下文和授权下载连接真实 FastAPI，通用文档、上传和问答仍连接 Node mock。
...
- Stage 3A Task 16：搜索卡片内按需引用上下文与授权下载（已交付），前文、命中片段和后文以纯文本展示，重新展开会重新授权；下载只导航到 Identity API，经实时授权后 `307` 到短时效对象地址并在新标签页打开，Web 不缓存或读取最终预签名 URL。Web 上传、资源详情与重试/删除、全文格式化预览、生成式回答和 Mock 退场仍在后续任务。
