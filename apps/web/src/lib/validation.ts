...
//
// 为什么不放在组件里：和 Day 8 抽 describeStatus 是同一条理由，这次是主动应用。
...
//   返回错误码    → 多一层间接，而这个应用没有 i18n 需求（Day 9 范围内）
...
export function validateEmail(value: string): FieldError {
  ...
  // 邮箱是否真实存在只有发一封信才知道——那是 Day 18 的验证邮件要做的事。
 ...
 * Day 18 后端做密码哈希时会再碰这个话题（那里的重点是 bcrypt/argon2 和撞库）。
 */
